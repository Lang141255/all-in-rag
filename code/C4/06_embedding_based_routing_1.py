...
# 初始化嵌入模型，并对路由描述进行向量化
embeddings = HuggingFaceEmbeddings(model_name="BAAI/bge-small-zh-v.15")
...
def route(info):
    # 对用户查询进行嵌入
    query_embeddings = embeddings.embed_query(info["query"])

    # 计算与各路由提示的余弦相似度
    similarity_scores = cosine_similarity([query_embeddings], route_prompt_embeddings)[0]
