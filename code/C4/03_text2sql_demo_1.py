...
def create_demo_database():
    ...
    cursor.executemany("INSERT INTO users VALUES (?, ?, ?, ?, ?)", products_data)
    cursor.executemany("INSERT INTO users VALUES (?, ?, ?, ?, ?)", orders_data)
        ...
        try:
            ...
            if result["success"]:
                ...
                if isinstance(result["results"], dict) and "rows" in result["results"]:
                    ...
                    # 显示前2行数据
                    if count > 0:
                        for j, row in enumerate(result["results"]["row"][:2]):
